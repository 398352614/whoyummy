  ...
  <div class="add-deduction">
    ...
    <div class="deduction-box">
      <el-form ref="form" :rules="rules" :model="form" label-width="150px">
        ...
        <el-form-item :label="$t('DebitType')" prop="deduct_type">
          <el-select
            v-model="form.deduct_type"
            :placeholder="$t('PleaseSelectDebitType')"
            style="width: 370px"
          >
            <el-option
              v-for="item in billDeductTypeList"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            >
        ...
        >
          <el-input
            ...
            :placeholder="$t('PleaseEnterTheDebitAmount')"
          ></el-input>
        </el-form-item>
        <el-form-item :label="$t('SystemNumber')" prop="remark">
          <el-input v-model="form.object_no"></el-input>
        </el-form-item>
  ...
  methods: {
    ...
    getDictionary() {
      this.$api.getDictionary().then((res) => {
        this.billDeductTypeList = res.data.billDeductTypeList;
      });
    },
    //货主列表-查询
    getMerchants() {
    ...
    onSubmit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.$api.postDeduct(this.form).then((res) => {
            if (res.code == 200) {
              this.form = {
                merchant_id: "",
                merchantName: "",
