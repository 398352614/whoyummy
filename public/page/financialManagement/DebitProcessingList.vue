  ...
  <div class="cash-recharge-list">
    <div class="conditional">
      ...
      <el-tooltip
        ...
        placement="top"
        :disabled="hasPerm('bill.merchant-deduct') ? true : false"
      >
        <div>
          <el-button
            :disabled="!hasPerm('bill.merchant-deduct')"
            :class="[
              hasPerm('bill.merchant-deduct') ? 'common' : 'common-disabled',
              'but',
            ]"
            size="small"
            :loading="this.$store.state.config.button_loading"
            @click="$router.push('AddDeduction')"
            >{{ $t("AddDeduction") }}</el-button
          >
        </div>
      </el-tooltip>
    </div>
    <el-table
      ...
      ref="table"
      :data="hasPerm('bill.merchant-deduct-index') ? tableData : []"
    ...
    >
      <template slot="empty" v-if="!hasPerm('bill.merchant-deduct-index')">
          ...
          <el-dropdown size="small" trigger="hover" placement="bottom">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-tooltip
                :content="$t('AdministratorActivate')"
                placement="top"
                :disabled="hasPerm('bill.merchant-deduct-index') ? true : false"
              >
                <div
                  :style="
                    hasPerm('bill.merchant-deduct-index') ? '' : 'cursor: not-allowed'
                  "
                >
                  <el-dropdown-item
                    @click.native="getDetailsVerify(scope.row.id)"
                    :disabled="!hasPerm('bill.merchant-deduct-index')"
                  >
                    <el-button
                      class="operation"
                      size="small"
              ...
              <el-tooltip
                :content="$t('AdministratorActivate')"
                placement="top"
                :disabled="hasPerm('bill.merchant-deduct-verify') ? true : false"
              >
                <div
                  :style="hasPerm('bill.merchant-deduct-verify') ? '' : 'cursor: not-allowed'"
                >
                  <el-dropdown-item
                    @click.native="getReview(scope.row.id)"
                    v-if="scope.row.verify_status == 1"
                    :disabled="!hasPerm('bill.merchant-deduct-verify')"
                  >
                    <el-button
                      class="operation"
                      size="small"
...
export default {
  ...
  methods: {
    ...
    getDeduct() {
      // 列表
      this.$api.getDeduct(this.pageParams).then((res) => {
