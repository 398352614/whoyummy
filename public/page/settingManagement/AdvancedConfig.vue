  ...
  <div class="basic-config">
    ...
    >
      <el-form-item :label="$t('RouteAssignRules')" prop="line_rule">
        <el-select
          v-model="senior.line_rule"
          @visible-change="handleChangeTemplate"
          @change="handleChangeLineRule"
          :placeholder="$t('PleaseSelectRouteAssignRules')"
        >
          <el-option
      ...
      <div class="template-box show-template-box">
        ...
        <div
          :class="{
            ...
            'address-template-check': senior.address_template_id === 1,
          }"
        >
          <el-form
            :model="template"
          ...
          <div class="address-template-radio">
            <el-radio v-model="senior.address_template_id" :label="1"
              >选择该模板</el-radio
            >
          </div>
        </div>
        ...
        <div
          :class="{
            'address-template': true,
            'address-template-check': senior.address_template_id === 2,
          }"
        >
          <el-form
            :model="template"
          ...
          <div class="address-template-radio">
            <el-radio v-model="senior.address_template_id" :label="2"
              >选择该模板</el-radio
            >
          </div>
        </div>
    ...
    <div class="save save-senior">
      <el-button
        v-if="hasPerm('company-config.update')"
        type="primary"
        @click="postCompanyConfigure"
    ...
    >
      ...
      <span slot="footer">
        <div class="template-radio">
          ...
          >
          <el-radio
            v-model="senior.address_template_id"
            :label="2"
            v-if="senior.line_rule !== 1"
            >选择该模板</el-radio
          >
      ...
      this.$refs.seniorForm.validate((valid) => {
        ...
        this.$api.postCompanyConfigure(this.senior).then((res) => {
          ...
          localStorage.setItem(
            "address_template_id",
            this.senior.address_template_id
          );
          // 线路分配规则
          ...
          // 重量
          this.$store.commit("globalData/setWeight", this.senior.weight_unit); // 设置重量单位
          // 设置地图
