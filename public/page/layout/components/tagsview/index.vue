...
export default {
  ...
  computed: {
    ...
    fileterAfterRouterMap () {
      // return this.$store.state.module.fileterAfterRouterMap[0] ? this.$store.state.module.fileterAfterRouterMap[0].children : []
      return []
    },
    routes () {
      return this.$store.state.module.permission.routes
    },
    routeTable() {
      return this.$store.state.routerData.routerMap;
    }
  },
  watch: {
    $route () {
    ...
    toLastView (visitedViews, view) {
      ...
      if (latestView) {
        ...
        if (view.name === 'Dashboard') {
          ...
          let defaultPagePath = this.routeTable[0].children[0].children.find(el => {
            return el.nav == 2
          })
          this.$router.push({name: defaultPagePath.name})
        }
      }
    },
    openMenu (tag, e) {
